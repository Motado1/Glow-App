/**
 * Sample data — a realistic two-state dataset for demos and for exercising the
 * app before real farms exist.
 *
 * Deliberately NOT loaded on install. `buildEmptyWorkspace()` in `seed.ts` is
 * what a fresh app gets; this is only produced when someone explicitly asks for
 * it from Account. Keeping the two apart is what stops a demo dataset from ever
 * landing on top of real farms.
 *
...
import { FOUNDING_ADMIN, type WorkspaceData } from './seed';

/** The founding admin plus a sample crew, so assignments have someone to point at. */
...
export function buildSampleData(): WorkspaceData {
  ...
  return { users: SAMPLE_USERS, farms, photos, submissions, problems, activity, notifications, boxInstallations };
}
